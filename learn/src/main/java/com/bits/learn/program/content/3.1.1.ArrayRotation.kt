...
fun rotateLeft(input : Array<Int>, offset : Int) : Array<Int> {
    com.bits.learn.program.trash.input("${Arrays.toString(input)} should be rotated $offset values to the left side")
    val result = input
    var _offset = offset
    while (_offset > 0){
    ...
    com.bits.learn.program.trash.output("The rotated array is ${result.contentToString()}")

    return result
}

fun rotateRight(input : Array<Int>, offset : Int){
    com.bits.learn.program.trash.input("${Arrays.toString(input)} should be rotated $offset values to the right side")
